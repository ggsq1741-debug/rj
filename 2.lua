...
v1.6.63|2025-12-25|用于脚本的Roblox UI库要查看源代码，请参阅官方GitHub存储库上的“ SRC/”文件夹。作者:FootageSus（影片，.ftgs，oftgs）Github：https：//github.com/Footagesus/WindUI.不和谐：https：//discord.gg/ftgs-development-hub-1300692552005189632.许可证：麻省理工学院
]]
...
SearchBarBorderTransparency=.65,
...
ah=ah or"Primary"
local am=al or(not ak and 16 or 99)
local an
if af and af~=""then
an=ac("ImageLabel",{
Image=ab.Icon(af)[1],
...
Size=UDim2.new(1,0,1,0),
Name="Special",
ImageTransparency=ah=="Secondary"and 0.88 or 1
}),
...
ad(ao.Frame,.12,{ImageTransparency=.90}):Play()
end)
...
ad(aq.Frame.Bar,0.28,{ImageTransparency=.88},Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
...
if aj.Window.NewElements then
af(ak.UIElements.SliderIcon.Frame.Thumb,.24,{ImageTransparency=.88,Size=UDim2.new(0,(aj.Window.NewElements and(ak.ThumbSize*2)or(ak.ThumbSize))+8,0,ak.ThumbSize+8)},Enum.EasingStyle.Quint,Enum.EasingDirection.Out):Play()
end
...
if not ap.Locked then
aj(ap.UIElements.Main.Frame,0.08,{ImageTransparency=.93}):Play()
end
end)
...
am(aB.Outline,0.04,{ImageTransparency=.88}):Play()
end)
